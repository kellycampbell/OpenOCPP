...
namespace chargelab {
    namespace {
        constexpr int kPriorityStartTransaction = 0;
        constexpr int kPriorityStopTransaction = 0;
        constexpr int kPriorityMeterValue = 5;
        constexpr int kClockDriftThresholdSeconds = 10;
        constexpr int kMillisecondsInDay = 1000*60*60*24;
        constexpr std::uint64_t kMeterValuesGroupId = 0x3735DB9FF7144036ull;
        constexpr int kPriorityMeterValuesNotification = 90;
    }

    TransactionModule2_0::TransactionModule2_0(
    ...
    void TransactionModule2_0::runStep(ocpp2_0::OcppRemote& remote) {
        ...
        if (connector_status_module_->getResetReasonHard()) {
            for (auto& x : active_transactions_) {
                ...
                stopTransaction(x.first, ocpp2_0::TriggerReasonEnumType::kResetCommand, ocpp2_0::ReasonEnumType::kImmediateReset, std::nullopt);
            }
        }
        ...
        for (auto& entry : active_transactions_) {
            ...
            if (!entry.first.has_value() || !entry.second->plugged_in) {
                ...
                if (elapsed > settings_->ConnectionTimeOut.getValue()*1000) {
                    ...
                    stopTransaction(entry.first, ocpp2_0::TriggerReasonEnumType::kEVConnectTimeout, ocpp2_0::ReasonEnumType::kTimeout, std::nullopt);
                    continue;
                }
            }

            if (shouldTakeReading(entry.second->last_sampled_reading_timestamp, settings_->MeterValueSampleInterval.getValue())) {
                addSampledReading(entry.second->last_sampled_reading_timestamp, entry.first, entry.second.value(),
                        ocpp2_0::TriggerReasonEnumType::kMeterValuePeriodic, ocpp2_0::ReadingContextEnumType::kSample_Periodic,
            ...
            if (shouldTakeReading(entry.second->last_sampled_ended_meter_values_timestamp, settings_->SampledDataTxEndedInterval.getValue())) {
                addEndedReading(entry.second->last_sampled_ended_meter_values_timestamp, entry.first, entry.second.value(),
                        ocpp2_0::ReadingContextEnumType::kSample_Periodic, settings_->StopTxnSampledData.getValue());
            }

            if (shouldTakeReading(entry.second->last_clock_aligned_reading_timestamp, settings_->ClockAlignedDataInterval.getValue())) {
                addSampledReading(entry.second->last_clock_aligned_reading_timestamp, entry.first, entry.second.value(),
                        ocpp2_0::TriggerReasonEnumType::kMeterValueClock, ocpp2_0::ReadingContextEnumType::kSample_Clock,
            ...
            if (shouldTakeReading(entry.second->last_clock_aligned_ended_meter_values_timestamp, settings_->AlignedDataTxEndedInterval.getValue())) {
                addEndedReading(entry.second->last_clock_aligned_ended_meter_values_timestamp, entry.first, entry.second.value(),
                        ocpp2_0::ReadingContextEnumType::kSample_Clock, settings_->AlignedDataTxEndedMeasurands.getValue());
            }
            ...
            if (entry.second->last_charging_status != std::make_optional(status)) {
                updateTransaction(entry.first, std::nullopt, std::nullopt,
                        ocpp2_0::TriggerReasonEnumType::kChargingStateChanged, platform_->systemClockNow());
            }
        }
        ...
        if (shouldTakeReading(last_non_transaction_meter_value_trigger_, settings_->ClockAlignedDataInterval.getValue())) {
            if (std::abs(last_trigger_delta) < kMillisecondsInDay) {
                for (auto const& entry : station_->getConnectorMetadata()) {
                    ...
                    auto meter_values = getMeterValues(entry.first, ocpp2_0::ReadingContextEnumType::kSample_Clock,
                            settings_->MeterValuesAlignedData.getValue(), last_non_transaction_meter_value_trigger_);
                    if (!meter_values.filtered.has_value())
                        continue;

                    pending_messages_module_->sendRequest2_0(
                            ocpp2_0::MeterValuesRequest {entry.first.id, std::move(meter_values.filtered.value())},
        ...
        for (auto& entry : active_transactions_) {
            ...
            if (entry.second->authorized && !entry.second->deauthorized && status->vehicle_connected) {
                power_management_module_->onActiveTransactionStarted(
                        entry.first->id, std::move(entry.second->charging_profile), entry.second->start_ts);
    ...
    TransactionModule2_0::onRequestStartTransactionReq(const ocpp2_0::RequestStartTransactionRequest &req) {
        auto const& metadata = station_->getConnectorMetadata();
    ...
    void TransactionModule2_0::onAuthorizeRsp(
    ...
    void TransactionModule2_0::onTransactionEventRsp(
            const std::string &unique_id,
                    ...
                    switch (value.idTokenInfo->status) {
                        ...
                        case ocpp2_0::AuthorizationStatusEnumType::kUnknown:
                            if (settings_->StopTransactionOnInvalidId.getValue()) {
                                if (set::containsAny(getStopPoints(),
                                    ...
                                    stopTransaction(entry.first, ocpp2_0::TriggerReasonEnumType::kDeauthorized,
                                            ocpp2_0::ReasonEnumType::kDeAuthorized, std::nullopt);
                                } else {
                                    // C15.FR.03
                                    deauthorizeTransaction(entry.first, entry.second.value(), ocpp2_0::TriggerReasonEnumType::kDeauthorized);
                                }
                            } else {
                                if (set::containsAny(getStopPoints(), ocpp2_0::TxStopPointValues::kEnergyTransfer)) {
                                    ...
                                    // C15.FR.03
                                    deauthorizeTransaction(entry.first, entry.second.value(), ocpp2_0::TriggerReasonEnumType::kChargingStateChanged);
                                }
                            }
                            break;
                    }
    ...
    TransactionModule2_0::onRequestStopTransactionReq(const ocpp2_0::RequestStopTransactionRequest &req) {
        for (auto& entry : active_transactions_) {
            if (!entry.second.has_value())
                continue;
            ...
            stopTransaction(entry.first, ocpp2_0::TriggerReasonEnumType::kRemoteStop, ocpp2_0::ReasonEnumType::kRemote, std::nullopt);
    ...
    TransactionModule2_0::onTriggerMessageReq(const ocpp2_0::TriggerMessageRequest &req) {
        if (req.requestedMessage == ocpp2_0::MessageTriggerEnumType::kMeterValues) {
            ...
            for (auto& entry : active_transactions_) {
                ...
                addSampledReading(platform_->systemClockNow(), entry.first, entry.second.value(),
                        ocpp2_0::TriggerReasonEnumType::kTrigger, ocpp2_0::ReadingContextEnumType::kTrigger,
                        settings_->MeterValuesSampledData.getValue());
    ...
    TransactionModule2_0::onGetTransactionStatusReq(const ocpp2_0::GetTransactionStatusRequest &request) {
    ...
    transaction_module2_0::TransactionContainer& TransactionModule2_0::startTransaction(
            std::optional<ocpp2_0::EVSEType> evse,
            ...
            std::optional<std::vector<ocpp2_0::SampledValueType>> original_meter_values,
            std::optional<std::vector<ocpp2_0::MeterValueType>> meter_values,
            std::optional<ocpp2_0::ChargingProfileType> const& charging_profile,
            std::optional<int> const& remote_start_id
    ) {
    ...
    bool TransactionModule2_0::updateTransaction(
            std::optional<ocpp2_0::EVSEType> evse,
            std::optional<ocpp2_0::IdTokenType> const& id_token,
            std::optional<ocpp2_0::IdTokenType> const& group_id_token,
            ocpp2_0::TriggerReasonEnumType const& trigger_reason,
    ...
    detail::PendingMessageWrapper TransactionModule2_0::generateStopRequest(
        ...
        auto const final_result = getMeterValues(evse, ocpp2_0::ReadingContextEnumType::kTransaction_End,
                settings_->StopTxnSampledData.getValue(), now);
        if (final_result.filtered.has_value()) {
            for (auto const &x : final_result.filtered.value())
    ...
    void TransactionModule2_0::stopTransaction(
            std::optional<ocpp2_0::EVSEType> const& evse,
            ocpp2_0::TriggerReasonEnumType const& trigger_reason,
            ocpp2_0::ReasonEnumType const& stopped_reason,
            std::optional<ocpp2_0::IdTokenType> id_token
    ...
    bool TransactionModule2_0::stopByIdTag(ocpp2_0::IdTokenType id_token) {
        for (auto& x : active_transactions_) {
            ...
            if (value.type == id_token.type && value.idToken == id_token.idToken) {
                ...
                if (set::containsAny(stop_points, ocpp2_0::TxStopPointValues::kAuthorized, ocpp2_0::TxStopPointValues::kPowerPathClosed)) {
                    ...
                    stopTransaction(x.first, ocpp2_0::TriggerReasonEnumType::kStopAuthorized, ocpp2_0::ReasonEnumType::kLocal, id_token);
                } else {
                    deauthorizeTransaction(x.first, x.second.value(), ocpp2_0::TriggerReasonEnumType::kStopAuthorized);
                }
    ...
    void TransactionModule2_0::deauthorizeTransaction(
        ...
        updateTransaction(evse_id, std::nullopt, std::nullopt, trigger_reason, platform_->systemClockNow());
    }

    bool TransactionModule2_0::stopByGroupId(ocpp2_0::IdTokenType id_token, std::optional<ocpp2_0::IdTokenType> const& group_id) {
        ...
        for (auto& x : active_transactions_) {
            ...
            if (value.type == group_id->type && value.idToken == group_id->idToken) {
                auto const& stop_points = getStopPoints();
                if (set::containsAny(stop_points, ocpp2_0::TxStopPointValues::kAuthorized, ocpp2_0::TxStopPointValues::kPowerPathClosed)) {
                    CHARGELAB_LOG_MESSAGE(info) << "Stopping transaction (group ID) - transaction ID: " << x.second->transaction_id;
                    stopTransaction(x.first, ocpp2_0::TriggerReasonEnumType::kStopAuthorized, ocpp2_0::ReasonEnumType::kLocal, id_token);
                } else {
                    deauthorizeTransaction(x.first, x.second.value(), ocpp2_0::TriggerReasonEnumType::kDeauthorized);
                }
    ...
    void TransactionModule2_0::processVehicleConnectedStateChanged(ocpp2_0::EVSEType const& evse, bool connected) {
        ...
        if (active_transactions_[evse].has_value()) {
            if (!connected) {
                if (set::containsAny(getStopPoints(), ocpp2_0::TxStopPointValues::kEVConnected, ocpp2_0::TxStopPointValues::kPowerPathClosed)) {
                    ...
                    stopTransaction(evse, ocpp2_0::TriggerReasonEnumType::kEVCommunicationLost, ocpp2_0::ReasonEnumType::kEVDisconnected, std::nullopt);
                }
            } else {
                updateTransaction(evse, std::nullopt, std::nullopt, ocpp2_0::TriggerReasonEnumType::kEVCommunicationLost, platform_->systemClockNow());
            }
        ...
        if (connected) {
            for (auto& x : active_transactions_) {
                ...
                updateTransaction(evse, std::nullopt, std::nullopt, ocpp2_0::TriggerReasonEnumType::kCablePluggedIn, now,
                        getMeterValues(evse, ocpp2_0::ReadingContextEnumType::kTransaction_Begin,
                                settings_->SampledDataTxStartedMeasurands.getValue(), now).filtered);
            ...
            if (!connector_status_module_->getPendingReset()) {
                ...
                if (set::contains(start_points, ocpp2_0::TxStartPointValues::kEVConnected)) {
                    auto const now = floorToSecond(platform_->systemClockNow());
                    auto const meter_values = getMeterValues(evse, ocpp2_0::ReadingContextEnumType::kTransaction_Begin,
                            settings_->SampledDataTxStartedMeasurands.getValue(), now);
                    auto const& result = startTransaction(evse, std::nullopt, std::nullopt,
                            ocpp2_0::TriggerReasonEnumType::kCablePluggedIn, false, now,
                            meter_values.original, meter_values.filtered);
        ...
        if (connected && !plug_and_charge_id.empty()) {
            ...
            for (auto const& x : pending_start_req_) {
                if (!x.first.has_value()) { found = true; break; }
                if (x.first->id == evse.id) {
                    if (!x.first->connectorId.has_value()) { found = true; break; }
                    if (x.first->connectorId.value() == evse.connectorId.value()) { found = true; break; }
                }
            }
            ...
            pending_start_req_[evse] = transaction_module2_0::PendingStartRequest {
                    platform_, evse,
                    ocpp2_0::IdTokenType {plug_and_charge_id, ocpp2_0::IdTokenEnumType::kLocal}
            };
        }
    }

    void TransactionModule2_0::processPendingStartRequest(ocpp2_0::OcppRemote& remote, std::optional<transaction_module2_0::PendingStartRequest>& pending) {
            ...
            if (treatAsConnected()) {
                if (!pending->pending_authorize_req.operationInProgress()) {
                    pending->pending_authorize_req.setWithTimeout(
                            ...
                            remote.sendAuthorizeReq(ocpp2_0::AuthorizeRequest {std::nullopt, pending->id_token})
                    );
                }
                return;
        ...
        for (auto& active : active_transactions_) {
            ...
            updateTransaction(active.first, pending->id_token, pending->group_id_token,
                    ocpp2_0::TriggerReasonEnumType::kAuthorized, platform_->systemClockNow());
            return;
        }
        ...
        for (auto const& entry : station_->getConnectorMetadata()) {
            ...
            if (set::contains(start_points, ocpp2_0::TxStartPointValues::kPowerPathClosed)) {
                ocpp2_0::TriggerReasonEnumType start_reason = pending->remote_start_id.has_value() ?
                        ocpp2_0::TriggerReasonEnumType::kRemoteStart : ocpp2_0::TriggerReasonEnumType::kCablePluggedIn;

                auto const now = floorToSecond(platform_->systemClockNow());
                auto const meter_values = getMeterValues(entry.first, ocpp2_0::ReadingContextEnumType::kTransaction_Begin,
                        settings_->SampledDataTxStartedMeasurands.getValue(), now);
                auto const& result = startTransaction(entry.first, pending->id_token, pending->group_id_token,
                        start_reason, true, now,
                        meter_values.original, meter_values.filtered,
                        pending->charging_profile, pending->remote_start_id);
                ...
                ocpp2_0::TriggerReasonEnumType start_reason = pending->remote_start_id.has_value() ?
                        ocpp2_0::TriggerReasonEnumType::kRemoteStart : ocpp2_0::TriggerReasonEnumType::kAuthorized;
        ...
        if (set::contains(start_points, ocpp2_0::TxStartPointValues::kAuthorized)) {
            ...
            auto const now = floorToSecond(platform_->systemClockNow());
            auto const& result = startTransaction(pending->evse, pending->id_token, pending->group_id_token,
                    start_reason, true, now, std::nullopt, std::nullopt, pending->charging_profile, pending->remote_start_id);
    ...
    void TransactionModule2_0::addSampledReading(
    ...
    void TransactionModule2_0::addEndedReading(
            SystemTimeMillis now,
            std::optional<ocpp2_0::EVSEType> const& evse,
            transaction_module2_0::TransactionContainer& entry,
            ocpp2_0::ReadingContextEnumType const& context,
        ...
        for (auto const& x : meter_values.filtered.value())
    ...
    bool TransactionModule2_0::shouldTakeReading(SystemTimeMillis& last, int interval_seconds) {
    ...
    ocpp2_0::ChargingStateEnumType TransactionModule2_0::getChargingState(
            std::optional<ocpp2_0::EVSEType> evse_id,
            transaction_module2_0::TransactionContainer& transaction
    ) {
        if (!evse_id.has_value())
    ...
    bool TransactionModule2_0::treatAsConnected() {
    ...
    bool TransactionModule2_0::isWebsocketConnected() {
        auto const websocket = platform_->ocppConnection();
        if (websocket == nullptr)
            return false;
    ...
    transaction_module2_0::MeterValuesResult TransactionModule2_0::getMeterValues(
            std::optional<ocpp2_0::EVSEType> const& evse,
            ocpp2_0::ReadingContextEnumType const& context,
            std::string const& measurands,
            SystemTimeMillis now
        ...
        return {
            ...
            std::vector<ocpp2_0::MeterValueType> {ocpp2_0::MeterValueType {now, std::move(sampled_values)}}
        };
    }

    std::set<ocpp2_0::TxStartPointValues> TransactionModule2_0::getStartPoints() {
    ...
    std::set<ocpp2_0::TxStopPointValues> TransactionModule2_0::getStopPoints() {
    ...
    std::unordered_set<ocpp2_0::MeasurandEnumType::Value> TransactionModule2_0::parseMeasurandsString(std::string const& measurands) {
    ...
    SystemTimeMillis TransactionModule2_0::floorToSecond(SystemTimeMillis ts) {
