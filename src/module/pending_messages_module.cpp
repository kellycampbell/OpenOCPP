...
namespace chargelab {
    namespace {
        constexpr int kUpdateCacheAndStatsFrequencySeconds = 15;
        constexpr int kMaxLiveMessages = 5;
        constexpr int kOfflineSizeLimitBytes = 10*1024;
        constexpr int kTargetDeleteRecordCount = 10;
        constexpr int kMaxStorageBlockSize = 50*1024;
        constexpr int kFlushToDiskFrequencyMillis = 30*60*1000;
    }
        ...
        if (!request_operation_.operationInProgress() && !live_queue_.empty()) {
            ...
            if (delete_message) {
                ...
                if (sendWithTransactionId(remote, *it)) {
                    it->attempts++;
                    request_operation_.setWithTimeout(settings_->DefaultMessageTimeout.getValue(), std::to_string(it->unique_id));
                    request_object_ = *it;
                }
            }
        }
            ...
            if (record.has_value()) {
                ...
                if (delete_message) {
                    ...
                    if (sendWithTransactionId(remote, record.value())) {
                        ...
                        pending_messages_changed_ = true;
                        request_operation_.setWithTimeout(settings_->DefaultMessageTimeout.getValue(), std::to_string(record->unique_id));
    ...
    bool PendingMessagesModule::sendWithTransactionId(ocpp1_6::OcppRemote& remote, detail::PendingMessageWrapper const& wrapper) {
        ...
        return remote.sendCall(std::to_string(wrapper.unique_id), wrapper.action_id1_6.value(), payload);
    }
    ...
    bool PendingMessagesModule::sendWithSequenceNumber(ocpp2_0::OcppRemote& remote, detail::PendingMessageWrapper const& wrapper) {
        ...
        return remote.sendCall(std::to_string(wrapper.unique_id), wrapper.action_id2_0.value(), payload);
    }
    ...
    void PendingMessagesModule::onCallRsp(const std::string &unique_id, const ocpp1_6::ResponseMessage<common::RawJson>& payload) {
        if (request_operation_ == unique_id) {
            ...
            if (std::holds_alternative<common::RawJson>(payload)) {
                ...
                auto handleRsp = [this](detail::PendingMessageWrapper const &wrapper) {
    ...
    int PendingMessagesModule::calculateAllPendingSavedInfoSize() {
        ...
        total_bytes += calculate_size(active_group_ids_) + 1;
        total_bytes += calculate_size(group_blacklist_) + 1;
        total_bytes += calculate_size(transaction_ids_) + 1;
        total_bytes += calculate_size(sequence_ids_) + 1;
        total_bytes += calculate_size(pending_messages_write_count_) + 1;

        for (auto const& x : live_queue_) {
            if (x.action_id1_6.has_value() && x.action_id1_6.value() == ocpp1_6::ActionId::kStartTransaction)
                continue;
            total_bytes += calculate_size(x) + 1;
        }
        ...
        for (auto const& supplier : saved_message_suppliers_) {
            ...
            (*supplier)([&](auto const& record) {
                total_bytes += calculate_size(record) + 1;
            });
        }
